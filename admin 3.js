...
async function openPatient(){
  ...
  unsub = onSnapshot(patientRef(id), (docSnap)=>{
    if(docSnap.exists()){
      ...
      cachedPatient = live;
      renderProgress(live);
      // IMPORTANT: do NOT force currentDay back to 1
      loadDayToForm(live);
    }
  });
}
...
async function saveDay(){
  ...
  await updateDoc(patientRef(id), { [`plan.${dayKey}`]: dayPlan });
...
function goDay(delta){
  ...
  setDayUI();
  if(cachedPatient) loadDayToForm(cachedPatient);
}
...
els.prevDay.addEventListener('click', (e)=>{ e.preventDefault(); goDay(-1); });
els.nextDay.addEventListener('click', (e)=>{ e.preventDefault(); goDay(+1); });
