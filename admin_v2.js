...
function loadDayToForm(p){
  const plan = p.plan[String(currentDay)];
...
async function openPatient(){
  ...
  if(!id) return setStatus("Pon un ID (ej: rss).");
  currentId = id;
...
async function saveDay(){
  ...
  setStatus(`Guardado Día ${currentDay}.`);
}
...
function copyLink(){
  const id = els.pid.value.trim();
  if(!id) return setStatus("Pon un ID.");
  const base = location.origin + location.pathname.replace('admin_v2.html','clean.html');
