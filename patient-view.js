...
async function init() {
    ...
    try {
        ...
        renderPatientInfo();
        renderMacros(); // Nueva función para renderizar macros
        renderCalendar();
...
function renderMacros() {
    // Calcular macros basados en peso y objetivo (ejemplo simplificado)
    const weight = patientData.weight || 70;
    const goal = (patientData.goal || '').toLowerCase();
    
    let calories, protein, carbs, fats;
    
    // Cálculos aproximados según objetivo
    ...
    // Si el paciente tiene datos personalizados, usarlos
    if (patientData.macros) {
        calories = patientData.macros.calories || calories;
        protein = patientData.macros.protein || protein;
        carbs = patientData.macros.carbs || carbs;
        fats = patientData.macros.fats || fats;
    }
        ...
        return `
            <div class="meal-item ${isCompleted ? 'completed' : ''}" data-meal="${mealKey}">
                ...
                <div class="meal-content">
                    <div class="meal-name">${mealIcons[index]} ${meal.name}</div>
