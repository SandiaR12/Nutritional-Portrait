...
function renderDaysEditor(existingDays = null) {
    ...
    for (let day = 1; day <= 15; day++) {
        ...
        dayEditor.innerHTML = `
            ...
            <div class="meal-editor" style="background:#f8f9fb;border:1.5px dashed rgba(0,102,255,.2);border-radius:12px;padding:16px;margin-top:8px">
                <h4 style="color:#004FCC;margin-bottom:12px">📸 Imágenes de referencia por comida</h4>
                <p style="font-size:.75rem;color:#6B7585;margin-bottom:14px">Sube una imagen para cada tiempo de comida. Aparecerá en la app del paciente cuando toque ese horario.</p>
                ${['desayuno','colacion1','comida','colacion2','cena'].map((meal, mi) => {
                    const mealNames = ['🌅 Desayuno','🍎 Colación AM','🍽️ Comida','🥤 Colación PM','🌙 Cena'];
                    const existingImg = dayData['img_' + meal] || '';
                    return \`<div style="margin-bottom:14px;padding-bottom:14px;border-bottom:1px solid rgba(0,0,0,.07)">
                        <div style="font-size:.78rem;font-weight:700;color:#3D4451;margin-bottom:8px">\${mealNames[mi]}</div>
                        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
                            <label style="display:inline-flex;align-items:center;gap:6px;background:#fff;border:1.5px solid rgba(0,102,255,.3);border-radius:8px;padding:7px 14px;cursor:pointer;font-size:.75rem;font-weight:600;color:#004FCC">
                                📁 Subir imagen
                                <input type="file" accept="image/*" style="display:none"
                                    onchange="window.handleDayMealImg(event,\${day},'\${meal}')">
                            </label>
                            <img id="img_prev_\${day}_\${meal}" src="\${existingImg}" 
                                style="width:56px;height:56px;object-fit:cover;border-radius:8px;border:1px solid rgba(0,0,0,.1);display:\${existingImg?'block':'none'}">
                            <button type="button" id="img_clear_\${day}_\${meal}"
                                onclick="window.clearDayMealImg(\${day},'\${meal}')"
                                style="display:\${existingImg?'inline-flex':'none'};align-items:center;gap:4px;background:rgba(255,59,48,.08);border:1px solid rgba(255,59,48,.2);color:#dc2626;border-radius:6px;padding:5px 10px;font-size:.72rem;cursor:pointer;">
                                ✕ Quitar
                            </button>
                            <input type="hidden" id="img_data_\${day}_\${meal}" value="\${existingImg}">
                        </div>
                    </div>\`;
                }).join('')}
            </div>
...
async function savePatient(e) {
    ...
    for (let day = 1; day <= 15; day++) {
        ...
        // Add images
        meals.forEach(meal => {
            const imgEl = document.getElementById('img_data_' + day + '_' + meal);
            if (imgEl && imgEl.value) dayObj['img_' + meal] = imgEl.value;
        });
...
// ── Image helpers for day/meal ────────────────────────────────────
window.handleDayMealImg = function(event, day, meal) {
    var file = event.target.files[0];
    if (!file) return;
    if (!file.type.startsWith('image/')) { alert('Solo imágenes (jpg, png, webp)'); return; }
    if (file.size > 800000) { alert('Imagen muy grande. Máximo 800KB.'); return; }
    var reader = new FileReader();
    reader.onload = function(e) {
        var b64 = e.target.result;
        var prev = document.getElementById('img_prev_' + day + '_' + meal);
        var data = document.getElementById('img_data_' + day + '_' + meal);
        var clr  = document.getElementById('img_clear_' + day + '_' + meal);
...
window.clearDayMealImg = function(day, meal) {
    var prev = document.getElementById('img_prev_' + day + '_' + meal);
    var data = document.getElementById('img_data_' + day + '_' + meal);
    var clr  = document.getElementById('img_clear_' + day + '_' + meal);
