...
// ── Image upload para tiempos de comida ───────────────────────────
window.handleMealImageUpload = function(dayKey, mealKey, input) {
    ...
    reader.onload = function(e) {
        ...
        // Preview
        const preview = document.getElementById('img_preview_' + dayKey + '_' + mealKey);
        if (preview) { preview.src = b64; preview.style.display = 'block'; }
        // Store in hidden input
        const hidden = document.getElementById('img_data_' + dayKey + '_' + mealKey);
        if (hidden) hidden.value = b64;
    };
    reader.readAsDataURL(file);
};

window.clearMealImage = function(dayKey, mealKey) {
    const preview = document.getElementById('img_preview_' + dayKey + '_' + mealKey);
    if (preview) { preview.src = ''; preview.style.display = 'none'; }
    const hidden = document.getElementById('img_data_' + dayKey + '_' + mealKey);
    if (hidden) hidden.value = '';
    const input = document.getElementById('img_input_' + dayKey + '_' + mealKey);
    if (input) input.value = '';
};

// Genera HTML de upload para cada tiempo de comida dentro de un día
window.getMealImageHTML = function(dayKey, mealKey, mealName, existingImg) {
    const hasImg = existingImg && existingImg.length > 0;
    return \`
    <div style="margin-top:6px">
      <div style="font-size:.7rem;color:#6B7585;margin-bottom:4px">📸 Imagen de referencia — \${mealName}</div>
      <div style="display:flex;align-items:center;gap:8px">
        <label style="
          display:inline-flex;align-items:center;gap:6px;
          background:#f0f7ff;border:1.5px dashed rgba(0,102,255,.3);
          border-radius:8px;padding:6px 12px;cursor:pointer;
          font-size:.72rem;font-weight:600;color:#004FCC;
        ">
          📁 Subir imagen
          <input id="img_input_\${dayKey}_\${mealKey}" type="file" accept="image/*"
            style="display:none"
            onchange="window.handleMealImageUpload('\${dayKey}', '\${mealKey}', this)">
        </label>
        \${hasImg ? \`<button onclick="window.clearMealImage('\${dayKey}', '\${mealKey}')"
          style="background:rgba(255,59,48,.08);border:1px solid rgba(255,59,48,.2);
          color:#dc2626;border-radius:6px;padding:5px 10px;font-size:.7rem;cursor:pointer;">✕ Quitar</button>\` : ''}
      </div>
      <img id="img_preview_\${dayKey}_\${mealKey}"
        src="\${hasImg ? existingImg : ''}"
        style="display:\${hasImg?'block':'none'};margin-top:6px;width:80px;height:80px;
        object-fit:cover;border-radius:8px;border:1px solid rgba(0,0,0,.1);">
      <input type="hidden" id="img_data_\${dayKey}_\${mealKey}" value="\${hasImg ? existingImg : ''}">
    </div>\`;
};
