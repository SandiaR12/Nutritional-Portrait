...
function renderDaysEditor(existingDays = null) {
    ...
    for (let day = 1; day <= 15; day++) {
        ...
        dayEditor.innerHTML = `
            ...
            <div style="background:#f0f7ff;border:1.5px dashed rgba(0,102,255,.25);border-radius:12px;padding:16px;margin-top:12px">
                <div style="font-size:.82rem;font-weight:700;color:#004FCC;margin-bottom:4px">📸 Imágenes de referencia</div>
                <div style="font-size:.72rem;color:#6B7585;margin-bottom:12px">Sube una imagen por tiempo de comida. El paciente la verá al abrir ese horario.</div>
                ${[['desayuno','🌅 Desayuno'],['colacion1','🍎 Colación AM'],['comida','🍽️ Comida'],['colacion2','🥤 Colación PM'],['cena','🌙 Cena']].map(([m,lbl]) => {
                    const ex = (dayData['img_'+m]||'');
                    return '<div style="margin-bottom:10px"><div style="font-size:.72rem;font-weight:600;color:#3D4451;margin-bottom:6px">'+lbl+'</div>'
                        +'<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">'
                        +'<label style="display:inline-flex;align-items:center;gap:5px;background:#fff;border:1.5px solid rgba(0,102,255,.3);border-radius:8px;padding:6px 12px;cursor:pointer;font-size:.72rem;font-weight:600;color:#004FCC">'
                        +'📁 Subir'
                        +'<input type="file" accept="image/*" style="display:none" onchange="window._imgUp(event,'+day+',''+m+'')">'
                        +'</label>'
                        +'<img id="ip_'+day+'_'+m+'" src="'+ex+'" style="width:48px;height:48px;object-fit:cover;border-radius:6px;border:1px solid rgba(0,0,0,.1);display:'+(ex?'block':'none')+'">'
                        +'<button type="button" onclick="window._imgClr('+day+',''+m+'')" style="display:'+(ex?'inline-flex':'none')+';background:rgba(255,59,48,.08);border:1px solid rgba(255,59,48,.2);color:#dc2626;border-radius:6px;padding:4px 8px;font-size:.68rem;cursor:pointer">✕ Quitar</button>'
                        +'<input type="hidden" id="id_'+day+'_'+m+'" value="'+ex+'">'
                        +'</div></div>';
                }).join('')}
            </div>
        `;
...
async function savePatient(e) {
    ...
    for (let day = 1; day <= 15; day++) {
        ...
        ['desayuno','colacion1','comida','colacion2','cena'].forEach(function(m){
            var el = document.getElementById('id_'+day+'_'+m);
            if (el && el.value) dayObj['img_'+m] = el.value;
        });
...
window._imgUp = function(evt, day, meal) {
    var f = evt.target.files[0];
    if (!f) return;
    if (!f.type.startsWith('image/')) { alert('Solo imágenes'); return; }
    if (f.size > 900000) { alert('Máximo 900KB'); return; }
    var r = new FileReader();
    r.onload = function(e) {
        var b = e.target.result;
        var p = document.getElementById('ip_'+day+'_'+meal);
        var d = document.getElementById('id_'+day+'_'+meal);
        var c = p && p.nextElementSibling && p.nextElementSibling.nextElementSibling;
        if (p) { p.src = b; p.style.display = 'block'; }
        if (d) d.value = b;
        if (c && c.tagName==='BUTTON') c.style.display = 'inline-flex';
    };
    r.readAsDataURL(f);
};
window._imgClr = function(day, meal) {
    var p = document.getElementById('ip_'+day+'_'+meal);
    var d = document.getElementById('id_'+day+'_'+meal);
